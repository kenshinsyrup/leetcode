...
public class Solution{
    class NumArray {
        ...
        // with BIT like: 308. Range Sum Query 2D - Mutable
        ...
        public void update(int i, int val) {
            ...
            // update nums array with val
            this.nums[i] = val;

            // update BIT with diff
            for(int nodeIdx = i + 1; nodeIdx <= len; nodeIdx += nodeIdx & (-nodeIdx)){
        ...
        // get [0:i] in nums sum, ie the sum from root node to current node(root node is dummy node for convience so nodeIdx should > 0)
        private int getSum(int i){
            int sum = 0;
            for(int nodeIdx = i + 1; nodeIdx > 0; nodeIdx -= nodeIdx & (-nodeIdx)){
                sum += this.bITree[nodeIdx];
            }
            return sum;
        }
