...
*/
    private int maxProfitByDPII(int[] prices) {
        ...
        int[][] dp = new int[len + 1][2];
        // Base case.
        // Given 0 Stock, given 0 Operation.
        dp[0][0] = 0; // No Stock at hand.
        dp[0][1] = Integer.MIN_VALUE; // Has Stock at hand. Impossible.
        // Given 0 Stock, given 1 Operation.
        dp[0][0] = 0; // No Stock at hand.
        dp[0][1] = Integer.MIN_VALUE; // Has Stock at hand. Impossible.


        for (int i = 0; i < len; i++) {
            // If we has no Stock when given i Stock and j Operations.
            dp[i + 1][0] = Math.max(dp[i][0], dp[i][1] + prices[i]);

            // If we has Stock
            dp[i + 1][1] = Math.max(dp[i][1], dp[i][0] - prices[i]); // Buy and Sell is called One Operation. Here to program, we make the  Buy looked as a Operation and Sell not. This make the Buy and Sell contribute One in k.
        }
    ...
    private int maxProfitByDP(int[] prices) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            // Do tranction.
            for (int j = 0; j < i; j++) {
                dp[i + 1] = Math.max(dp[i + 1], dp[j] + prices[i] - prices[j]);
            }
        }
